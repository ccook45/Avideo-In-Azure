#!/bin/bash
sudo apt update && sudo apt-get -y install apache2 php libapache2-mod-php php-mysql php-curl php-gd php-intl mysql-server mysql-client ffmpeg git libimage-exiftool-perl php-mbstring php-gettext python && cd /var/www/html && sudo git clone https://github.com/WWBN/AVideo-Encoder.git && sudo curl -L https://yt-dl.org/downloads/latest/youtube-dl -o /usr/local/bin/youtube-dl && sudo chmod a+rx /usr/local/bin/youtube-dl && sudo a2enmod rewrite && sudo phpenmod mbstring && sudo systemctl restart apache2
...
sudo sed -z 's/max_execution_time = 30/max_execution_time = 7200/' -i /etc/php/7.2/apache2/php.ini
sudo sed -z 's/memory_limit = 128M/memory_limit = 512M/' -i /etc/php/7.2/apache2/php.ini
sudo service apache2 restart
echo done